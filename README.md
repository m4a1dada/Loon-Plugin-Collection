...
本项目收集并维护 Loon 去广告插件，针对各款 iOS App 内置广告进行精准拦截。采用域名级阻断 + URL 正则重写 + Script 脚本 + MITM 解密四层防护策略，覆盖开屏广告、横幅广告、悬浮窗广告、弹窗广告、信息流广告等类型。
...
1. 打开 Loon → 配置 → 插件
2. 点击右上角 + → 从 URL 添加
3. 粘贴插件直链（推荐使用 jsdelivr CDN 链接），确认添加
4. 开启插件开关并启用 MITM
...
| 脚本注入 | Script http-response | 修改响应体，删除广告数据字段 |
